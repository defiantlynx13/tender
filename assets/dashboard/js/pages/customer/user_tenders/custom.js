jQuery(document).ready(function($){
    ...
    function fetch_data_current_tenders()
    {
        $('#current_tenders_tbl').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url"  : myData.ajax_url+'?action=get_all_current_tenders_data&user_id='+myData.user_id
            },
    ...
    $(document).on('click','#user_propose_price_confirm_btn',function () {
        var customer_propose_form=$('#customer_propose_form');
        var tender_price_customer=$('#tender_price_customer');
        customer_propose_form.find(".tender_price_customer_error").html("").addClass("hidden");

        allFormsData.tender_price_customer = tender_price_customer.val();

        let tender_Data = {
            tender_price_customer: allFormsData.tender_price_customer,
        };

        let rules = {
            tender_price_customer: 'required|numeric',
        };
        ...
        if (validation.fails())
        {
            ...
            var tender_price_customer_validation_errors = validation.errors.get('tender_price_customer');

            if (tender_price_customer_validation_errors && tender_price_customer_validation_errors.length > 0)
            {
                customer_propose_form.find(".tender_price_customer_error").html(validation.errors.errors['tender_price_customer'][0]).removeClass("hidden");
                traverse(document.body);
            }
        }
        else
    ...
    $(document).on('click','#request_tender_btn',function () {
        $('#tender_price_advisor').val($(this).data('tender_price'));
        $('.tender_price_advisor_msg').text(Num2persian($(this).data('tender_price'))+"(ریال)");
        $('#user_propose_price_confirm_btn').attr('data-tender_id',$(this).data('tender_id'));
        $('#tender_tools_advisor').FancyFileUpload({
            url : myData.ajax_url,
            params :
                {
                    action :'tnd_customer_tender_request_tool_file' ,
            ...
            uploadcompleted :function(e, data)
            {
                ...
                $.post(
                    myData.ajax_url,
                    {
                        action : 'add_customer_proposed_price',
                        customer_proposed_price: $('#tender_price_customer').val(),
                        user_id:  myData.user_id,
    ...
    function fetch_data_requested_tenders()
    {
        $('#requested_tenders_tbl').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url"  : myData.ajax_url+'?action=get_all_requested_tenders_data&user_id='+myData.user_id
            },
            "fnDrawCallback" : function() {
                traverse(document.body);
            }
    ...
    function fetch_data_winned_tenders()
    {
        $('#winned_tenders_tbl').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url"  : myData.ajax_url+'?action=get_all_winned_tenders_data&user_id='+myData.user_id
            },
            "fnDrawCallback" : function() {
                traverse(document.body);
            }
