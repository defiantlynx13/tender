jQuery(document).ready(function($){
    ...
    var update_corporation_data_form=$('#update_corporation_data');
    var corporation_name = $('#corporation_name');
    // var corporation_grade = $("input[type='radio'][name='corporation_grade']");
    var update_corporation_data_btn = $('#update_corporation_data_btn');
    var corporation_files_count=0;
    $('#corporation_files').FancyFileUpload({
        url : myData.ajax_url,
        params :
            {
                action :'tnd_customer_profile_corp_files_data' ,
        ...
        {
            corporation_files_count++;
        },
        uploadcompleted :function(e, data)
        {
            update_corp_name_grade();
        },
        uploadcancelled:function(e, data)
        {
            ...
            update_corporation_data_btn.find("i").removeClass('fa fa-circle-o-notch fa-spin');
            update_corporation_data_btn.prop('disabled', false);
        }
    });

    corporation_name.keydown(function (e) {
        if (e.keyCode == 13) {
            update_corporation_data_btn.trigger('click');
            e.stopPropagation();
            return false;
        }
    });

    update_corporation_data_btn.on('click',function (event) {
            update_corporation_data_form.find(".corporation_name_error").html("").addClass("hidden");
            update_corporation_data_form.find(".corporation_grade_error").html("").addClass("hidden");

            allFormsData.corporation_name = corporation_name.val();
            allFormsData.corporation_grade = $("input[type='radio'][name='corporation_grade']:checked").val();

            let corp_Data = {
                corporation_name: allFormsData.corporation_name,
                corporation_grade: allFormsData.corporation_grade,
            };

            let rules = {
                corporation_name: 'required',
                corporation_grade: 'required',
            };
            ...
            if (validation.fails())
            {
                var corporation_name_validation_errors = validation.errors.get('corporation_name');
                if (corporation_name_validation_errors && corporation_name_validation_errors.length > 0)
                {
                    update_corporation_data_form.find(".corporation_name_error").html(validation.errors.errors['corporation_name'][0]).removeClass("hidden");
                    traverse(document.body);
                }

                var corporation_grade_validation_errors = validation.errors.get('corporation_grade');
                if (corporation_grade_validation_errors && corporation_grade_validation_errors.length > 0)
                {
                    update_corporation_data_form.find(".corporation_grade_error").html(validation.errors.errors['corporation_grade'][0]).removeClass("hidden");
                    traverse(document.body);
                }
            ...
            else
            {
                ...
                update_corporation_data_form.find(".update_pass_current_pass_error").html("").addClass("hidden");
                update_corporation_data_btn.find("i").addClass(' icon-check2 fa fa-circle-o-notch fa-spin');
                update_corporation_data_btn.prop('disabled', true);

                $('#corp_data_update_confirm').modal('show');

            }

        });

    $('#corp_data_update_confirm_ok_btn').on('click',function (event) {
        $('#corp_data_update_confirm').modal('hide');
        $(".ff_fileupload_actions button.ff_fileupload_start_upload ").click();
    });

    $('#corp_data_update_confirm_cancel_btn').on('click',function (event) {
        var update_corporation_data_btn = $('#update_corporation_data_btn');
        update_corporation_data_btn.find("i").removeClass('fa fa-circle-o-notch fa-spin');
        update_corporation_data_btn.prop('disabled', false);
    });
    ...
    function update_corp_name_grade()
    {
        $.post(
            myData.ajax_url,
            {
                action : 'tnd_customer_profile_corp_name_grade_data',
                corporation_name: corporation_name.val(),
                corporation_grade: $("input[type='radio'][name='corporation_grade']:checked").val(),
                user_id:  myData.user_id
            },
            function (response)
            {
                if (response.status == 'true')
                {
                    $.notify(
                        {
                            message: 'اطلاعات شرکت شما برای کارشناس مربوطه ارسال گردید! پس از تایید مدارک ، می توانید در مناقصات شرکت نمایید!'
                        },
                        {
                            type: 'success',
                            placement: {
                ...
                else
                {
                    ...
                    $.notify(
                        {
                            message: 'نام یا گرید شرکت وارد نشده است.'
                        },
                        {
                            type: 'danger',
                            placement: {
