...
use Tender_Shop_Dir\Includes\PageHandlers\Factory_Profile_Handler;
use Tender_Shop_Dir\Includes\PageHandlers\Factory_Tender_Handler;
use Tender_Shop_Dir\Includes\PageHandlers\Factory_Tender_Managment_Handler;
use Tender_Shop_Dir\Includes\PageHandlers\Shop_Managment_Handler;
use Tender_Shop_Dir\Includes\PageHandlers\Shop_Profile_Handler;
use Tender_Shop_Dir\Includes\PageHandlers\Shop_Tender_Handler;
use Tender_Shop_Dir\Includes\PageHandlers\Shop_Tender_Managment_Handler;
 ...
 * @package    Tender_Shop_Dir\Includes\Init
 * @author     Your_Name <youremail@nomail.com>
 * @see        \Tender_Shop_Dir\Includes\PageHandlers\First_Page_Handler
 * @see        \Tender_Shop_Dir\Includes\PageHandlers\Second_Page_Handler
 */
class Router {
	...
	private function set_default_routes() {
        $this->routes = [
            '/shop_managment'        => Shop_Managment_Handler::class,
            '/shop_profile'        => Shop_Profile_Handler::class,
            '/factory_profile'        => Factory_Profile_Handler::class,
            '/shop_tender_managment'        => Shop_Tender_Managment_Handler::class,
            '/factory_tender_managment'        => Factory_Tender_Managment_Handler::class,
            '/shop_tenders'        => Shop_Tender_Handler::class,
            '/factory_tenders'        => Factory_Tender_Handler::class,
        ];
	}
	...
	private function get_current_route() {
		$web_protocol= ( isset( $_SERVER['HTTPS']) && 'on' === $_SERVER['HTTPS'] )? 'https://' : 'http://';
        $actual_link=$web_protocol.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
