...
class Utility {
	...
	public static function load_template( $template, $params = array(), $type = 'admin' ) {
		...
		$base_path      = 'admin' === $type ? Tender_Shop_TPL_ADMIN : Tender_Shop_TPL_FRONT;
    ...
    public static function check_customer_send_tender_request( $customer_id,$tender_contributors)
    {
        $temp=false;
        foreach ( $tender_contributors as $key => $val)
        {
            if ((int)$val['customer_id'] === intval($customer_id))
            {
                $temp=true;
                break;
            }
